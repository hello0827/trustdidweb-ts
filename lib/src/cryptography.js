...
import { bytesToHex, createDate } from "./utils.js";
import { base58btc } from "multiformats/bases/base58";
import { canonicalize } from 'json-canonicalize';
import { createHash } from 'node:crypto';
export const createSigner = (vm, useStatic = true) => {
    return async (doc, challenge) => {
        try {
            const proof = {
...
export const generateEd25519VerificationMethod = async (purpose) => {
...
export const generateX25519VerificationMethod = async (purpose) => {
    const privKey = ed.utils.randomPrivateKey();
    const pubKey = await ed.getPublicKeyAsync(privKey);
