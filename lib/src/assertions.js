...
import { bytesToHex, createSCID, deriveHash, resolveVM } from "./utils.js";
import { canonicalize } from 'json-canonicalize';
import { createHash } from 'node:crypto';
const isKeyAuthorized = (verificationMethod, updateKeys) => {
    if (process.env.IGNORE_ASSERTION_KEY_IS_AUTHORIZED)
        return true;
...
const isWitnessAuthorized = (verificationMethod, witnesses) => {
    if (process.env.IGNORE_WITNESS_IS_AUTHORIZED)
        return true;
...
export const documentStateIsValid = async (doc, proofs, updateKeys, witnesses = []) => {
    if (process.env.IGNORE_ASSERTION_DOCUMENT_STATE_IS_VALID)
        return true;
    let i = 0;
    while (i < proofs.length) {
        ...
        const verified = await ed.verifyAsync(bytesToHex(sig), bytesToHex(input), bytesToHex(publicKey.slice(2)));
        if (!verified) {
            return false;
        }
        i++;
...
export const hashChainValid = (derivedHash, logEntryHash) => {
    if (process.env.IGNORE_ASSERTION_HASH_CHAIN_IS_VALID)
        return true;
    return derivedHash === logEntryHash;
};
export const newKeysAreValid = (updateKeys, previousNextKeyHashes, nextKeyHashes, previousPrerotation, prerotation) => {
    if (process.env.IGNORE_ASSERTION_NEW_KEYS_ARE_VALID)
        return true;
...
export const scidIsFromHash = async (scid, hash) => {
    if (process.env.IGNORE_ASSERTION_SCID_IS_FROM_HASH)
        return true;
