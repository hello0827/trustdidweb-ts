...
import { clone, collectWitnessProofs, createDate, createDIDDoc, createSCID, deriveHash, findVerificationMethod, normalizeVMs } from "./utils.js";
import { BASE_CONTEXT, METHOD, PLACEHOLDER, PROTOCOL } from './constants.js';
import { documentStateIsValid, hashChainValid, newKeysAreValid, scidIsFromHash } from './assertions.js';
...
export const resolveDID = async (log, options = {}) => {
    ...
    for (const entry of resolutionLog) {
        ...
        if (version === '1') {
            ...
            meta.nextKeyHashes = params.nextKeyHashes ?? [];
            ...
            if (meta.nextKeyHashes) {
                nextKeyHashes = meta.nextKeyHashes;
            }
        }
        else {
            // version number > 1
            if (Object.keys(data).some((k) => k === 'value')) {
                newDoc = data.value;
            }
            else {
...
export const updateDID = async (options) => {
    const { log, updateKeys, context, verificationMethods, services, alsoKnownAs, controller, domain, nextKeyHashes, prerotation, witnesses, witnessThreshold } = options;
    ...
    if (meta.witnesses && meta.witnesses.length > 0) {
        const witnessProofs = await collectWitnessProofs(meta.witnesses, [...log, logEntry]);
        if (witnessProofs.length > 0) {
